<template>
  <div class="food_container">
        ...
        <transition name="showlist" v-show="category">
          <section v-show="sortBy == 'food'" class="category_container sort_detail_type">
            <section class="category_left">
              <ul>
                <li class="category_left_li" @click="selectCategoryName(item.id,index)" v-for="(item,index) in category" :key="index" :class="{category_active:restaurant_category_id == item.id}">
                  <section>
                    <img :src="imgBaseUrl+subImgUrl(item.image_url)" v-if="index" alt="图片" class="category_icon">
                    <span>{{item.name}}</span>
                  </section>
                  <section>
            ...
            <section class="category_right">
              <ul>
                <li class="category_right_li" @click="getCategoryIds(item.id,item.name)" v-for="(item,index) in categoryDetail" :key="item.id" :class="{category_right_choosed:restaurant_category_ids == item.id || (!restaurant_category_ids)&&index == 0}">
                  <span>{{item.name}}</span>
              ...
              <ul class="filter_ul">
                <li class="filter_li" v-for="item in Delivery" :key="item.id" @click="selectDeliveryMode(item.id)">
                  <svg :style="{opacity: (item.id == 0)&&(delivery_mode !== 0)? 0: 1}">
                    <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="delivery_mode == item.id? '#selected':'#fengniao'" />
                  </svg>
    ...
    <section class="shop_list_container">
      <shop-list :restaurantCategoryId="restaurant_category_id" :restaurantCategoryIds="restaurant_category_ids" :sortByType='sortByType' :deliveryMode="delivery_mode" :confirmSelect="confirmStatus" :supportIds="support_ids" v-if="latitude" @DidConfirm="clearAll"></shop-list>
    </section>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 点击某个排序方式，获取事件对象的data的值，并根据获取的值重新获取数据渲染
    sortList(event) {
    ...
    selectDeliveryMode(id) {
      // delivery_mode为空时，选中当前项，并且filterNum+1
      if (this.delivery_mode == null) {
        this.filterNum++;
        this.delivery_mode = id;
      }
      // delivery_mode为当前已有值时，清空所有项，并且filterNum-1
