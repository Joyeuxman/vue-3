...
export const sendMobile = (sendData, captcha_code, type, password) => fetch('/api/v1/mobile/verify_code/send', {
	action: "send",
	captcha_code,
	[type]: sendData,
	type: "sms",
...
export const getRemark = (id, sig) => fetch('/api/v1/carts/' + id + '/remarks', {
	sig
});
...
export const getAddress = (id, sig) => fetch('/api/v1/carts/' + id + '/addresses', {
	sig
});
...
export const searchNearby = keyword => fetch('/api/v1/pois', {
	type: 'nearby',
	keyword
});
...
export const postAddAddress = (userId, address, address_detail, geohash, name, phone, phone_bk, poi_type, sex, tag, tag_type) => fetch('/api/v1/users/' + userId + '/addresses', {
	address,
	address_detail,
	geohash,
	name,
...
export const rePostVerify = (cart_id, sig, type) => fetch('/api/v1/carts/' + cart_id + '/verify_code', {
	sig,
	type,
}, 'POST');
...
export const validateOrders = ({
	...
	validation_code,
	validation_token
}) => fetch('/api/v1/users/' + user_id + '/carts/' + cart_id + '/orders', {
	address_id,
	come_from: "mobile_web",
	deliver_time: "",
	description,
...
export const vipCart = (id, number, password) => fetch('/api/member/v1/users/' + id + '/delivery_card/physical_card/bind',{
	number,
	password
}, 'POST')
...
export const getOrderList = (user_id, offset) => fetch('/api/bos/v2/users/' + user_id + '/orders', {
	limit: 10,
	offset,
});
...
export const getOrderDetail = (user_id, orderid) => fetch('/api/bos/v1/users/' + user_id + '/orders/' + orderid + '/snapshot');
...
export const getSearchAddress = (keyword) => fetch('/api/v1/pois',{
	keyword:keyword,
	type:'nearby'
})
 ...
 * 改密码
 */
export const changePassword = (username, oldpassWord, newpassword, confirmpassword, captcha_code) => fetch('/api/v2/changepassword', {username, oldpassWord, newpassword, confirmpassword, captcha_code}, 'POST');
