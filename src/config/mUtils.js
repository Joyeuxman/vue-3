...
const getStyle = (element, attr, NumberMode = 'int') => {
  let target;
  // scrollTop 获取方式跟其他的不同，它不属于style，而且只有document.body才能用这个属性
  if (attr === 'scrollTop') {//属性为scrollTop
  ...
  const moveEnd = () => {
    requestFram = requestAnimationFrame(() => {
      if (document.body.scrollTop != oldScrollTop) {
        oldScrollTop = document.body.accessKey.scrollTop;
        loadMore();
        moveEnd();
      }
      else {
...
// 显示返回顶部按钮，开始、结束、运动 三个运动过程中调用 函数判断是否到达目标点
...
export { setStore, getStore, removeStore, getStyle, loadMore, showBack, animate }